...
Scope {
  ...
  onCurrentTextChanged: {
    Logger.i("LockContext", "currentText changed: length=" + currentText.length + ", val='" + currentText + "'");
    for (var i = 0; i < currentText.length; i++) {
      Logger.i("LockContext", "  char " + i + ": '" + currentText.charAt(i) + "' (code: " + currentText.charCodeAt(i) + ")");
    }
    if (currentText !== "") {
      showInfo = false;
      showFailure = false;
