...
PanelWindow {
  ...
  function applyCard(filePath) {
    ...
    var outputName = root.screen ? root.screen.name : "";

    if (Utils.isVideo(fileName, filterVideos)) {
      // Local extension to the upstream plugin: route video files through
      // mpvpaper via the wallcards-video helper. Keeps the UX inside this
      // plugin even though the upstream applyCard() no-ops for videos.
      var setProc = processComponent.createObject(null, {
        "command": ["/home/ldzbeta/.local/bin/wallcards-video", "set", outputName, filePath]
      });
      setProc.exited.connect(function () { setProc.destroy(); });
      setProc.running = true;
    } else {
      // Stop any video wallpaper before reverting to an image so mpvpaper
      // doesn't keep painting on top of Noctalia's wallpaper layer.
      var clearProc = processComponent.createObject(null, {
        "command": ["/home/ldzbeta/.local/bin/wallcards-video", "clear"]
      });
      clearProc.exited.connect(function () { clearProc.destroy(); });
      clearProc.running = true;

      var screen = Settings.data.wallpaper.setWallpaperOnAllMonitors ? undefined : outputName;
